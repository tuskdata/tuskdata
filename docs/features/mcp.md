...
| `advise(connection_id)` | Database health findings with the SQL to run (unindexed foreign keys, sequential-scan-heavy tables, unused/duplicate indexes, dead tuples, missing statistics, slowest queries). Nothing is applied. |
